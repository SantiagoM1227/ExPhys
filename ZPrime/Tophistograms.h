...
void TopAnalysis::define_histograms()
{
  ...
  hist_invmass2     = new TH1F("hist_invmass2", "Invariant mass for 2 jets;M_{inv}(2j);Events",50,0,500);
  hist_invmass3     = new TH1F("hist_invmass3", "Invariant mass for 2 jets;M_{inv}(3j);Events",25,0,1200);
  hist_invmassZ     = new TH1F("hist_invmassZ", "Invariant mass for 2 jets;M_{inv}(I,E_T^{miss},4j);Events",40,0,4000);
...
void TopAnalysis::FillOutputList()
{
  ...
  GetOutputList()->Add(hist_invmass2);
  GetOutputList()->Add(hist_invmass3);
  GetOutputList()->Add(hist_invmassZ);
...
void TopAnalysis::FillHistogramsGlobal( double m, float w , TString s)
{
  ...
  if (s.Contains("hist_invmass2")) hist_invmass2->Fill(m,w);

  if (s.Contains("hist_invmass3")) hist_invmass3->Fill(m,w);

  if (s.Contains("hist_invmassZ")) hist_invmassZ->Fill(m,w);
}
