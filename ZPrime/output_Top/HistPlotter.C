...
std::string histname = "hist_invmass3";
float lumi = 1000.;
...
/*
  ...
  //h_signal->Rebin(rebin);
  //Rescale signal
  h_signal->Scale(1e3*lumi*signalmap["ggH125_WW2lep"]);
  h_signal->SetLineStyle(7);
  h_signal->SetLineWidth(3);
  h_signal->SetLineColor(kPink-8);
*/
  ...
  h_stack->SetMaximum(200+h_stack->GetMaximum());
  ...
  leg->AddEntry(h_signal,("Z'(" + signalmass + " GeV)#rightarrow t#bar{t} x 10^{4}").c_str(),"l");
  leg->Draw();
