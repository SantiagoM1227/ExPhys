...
std::string histname = "hist_Hmass";
float lumi = 1000.;
float ISignal = 0;
float IBackground = 0;
...
void HistPlotter(){
  ...
  //TFile *f_sig_VBF = TFile::Open("VBFH125_WW2lep.root");
  //TH1F* h_signal_VBF = (TH1F*) f_sig_VBF->Get(histname.c_str());
  //h_signal->Rebin(rebin);
  //Rescale signal
  
  TFile *f_sig = TFile::Open("ggH125_WW2lep.root");
  TH1F* h_signal = (TH1F*) f_sig->Get(histname.c_str());
  //h_signal->Rebin(rebin);
  //Rescale signal
  

  //TH1F *h_signal = (TH1F*) h_signal_VBF->Clone();
  //h_signal->Add(h_signal_gg);
  
  
  ISignal = h_signal->Integral(10,140);
  h_signal->Scale(1e1*lumi*signalmap["ggH125_WW2lep"]);
  ...
  h_stack->SetMaximum(1.5*h_stack->GetMaximum());
  ...
  TLegend *leg = new TLegend(0.6, 0.6, 0.8, 0.95);
  leg->SetBorderSize(0);
  leg->SetTextFont(42);
  ...
  leg->AddEntry(h_signal,"H(125 GeV)#rightarrow WW x 10","l");
  leg->Draw();

  std::string outfile = histname.substr(0, std::string::npos);
  if(logy) can->SaveAs(("./histograms/"+outfile+"_log.pdf").c_str());
  else can->SaveAs(("./histograms/"+outfile+".pdf").c_str());

  cout<<"El valor de la señal es "<<ISignal<<endl;
  cout<<"El valor del background es "<<IBackground<<endl;
  cout<<"La significancia para busquedas es "<<ISignal/pow(IBackground,0.5)<<endl;
  cout<<"La significancia para mediciones es "<<ISignal/pow(ISignal+IBackground,0.5)<<endl;
  cout<<"La pureza de la señal es "<<ISignal/(ISignal+IBackground)<<endl;
}
...
void groupSamples(){
  ...
  while(!samples.eof()){
    ...
    if(sample.Contains("ggH125_WW2lep")  ) signalmap[sample] = xsec*nevt/(red_eff*sumw);
    if(sample.Contains("VBFH125_WW2lep")  ) signalmap[sample] = xsec*nevt/(red_eff*sumw);
...
void makeGroupHist(){
  ...
  IBackground += h_db->Integral(0,300);
  ...
  IBackground += h_drellyan->Integral(0,300);
  ...
  IBackground += h_wjets->Integral(0,300);
  ...
  IBackground += h_zjets->Integral(0,300);
  ...
  IBackground += h_singletop->Integral(0,300);
  ...
  IBackground += h_ttbar->Integral(0,300);
}
