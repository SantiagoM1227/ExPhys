...
Bool_t TopAnalysis::Process(Long64_t entry)
{
    ...
    TLorentzVector Hrecon = TLorentzVector();
    ...
    float m_vis = m_ll;
    float InvMassHrecon = Hrecon.M()/1000.;
    ...
    hist_deltaphi->Fill(TMath::Abs(lep_0.DeltaPhi(lep_1)),weight);
    hist_ptleptons->Fill(dilep.Pt()/1000.,weight);
    hist_Hmass->Fill(InvMassHrecon,weight);
