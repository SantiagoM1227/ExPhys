...
void TopAnalysis::define_histograms()
{
  ...
  hist_trailleptpt   = new TH1F("hist_trailleptpt",  "Leading Lepton Transverse Momentum;p_{T}^{traillep} [GeV];Leptons", 20, 0, 200);
  hist_traillepteta  = new TH1F("hist_traillepteta", "Leading Lepton Pseudorapidity; #eta^{traillep}; Leptons", 30, -3, 3);
  hist_trailleptE    = new TH1F("hist_trailleptE",   "Leading Lepton Energy; E^{traillep} [GeV]; Leptons", 15, 0, 300);
  hist_trailleptphi  = new TH1F("hist_trailleptphi", "Leading Lepton Azimuthal Angle ; #phi^{traillep}; Leptons", 32, -3.2, 3.2);
  hist_trailleptch   = new TH1F("hist_trailleptch",  "Leading Lepton Charge; Q^{traillep}; Leptons", 7, -1.75, 1.75);
  hist_trailleptID   = new TH1F("hist_trailleptID",  "Leading Lepton Absolute PDG ID; |PDG ID|^{traillep}; Leptons",  31, -0.5, 30.5);
  hist_traillept_ptc  = new TH1F("hist_traillept_ptc", "Leading Lepton Relative Transverse Momentum Isolation; ptconerel30^{traillep}; Leptons", 40, -0.1, 0.4);
  hist_trailleptetc  = new TH1F("hist_trailleptetc", "Leading Lepton Relative Transverse Energy Isolation; etconerel20^{traillep}; Leptons", 40, -0.1, 0.4);
  hist_traillepz0    = new TH1F("hist_traillepz0",   "Leading Lepton z0 impact parameter; z_{0}^{traillep} [mm]; Leptons", 40, -1, 1);
  hist_traillepd0    = new TH1F("hist_traillepd0",   "Leading Lepton d0 impact parameter; d_{0}^{traillep} [mm]; Leptons", 40, -1, 1);
  hist_deltaphi      = new TH1F("hist_deltaphi","Delta Phi between leptons; |#Delta#phi_{ll}|; Leptons", 8, 0, 1.6);
  hist_ptleptons     = new TH1F("hist_ptleptons","Transverse momentun of the two leptons; p_{T,ll}; Leptons", 10, 0, 200);
  hist_Hmass         = new TH1F("hist_Hmass","Reconstructed transverse mass for Higgs; M_{T}_{H}; Leptons", 20, 60, 300);
}
...
void TopAnalysis::FillOutputList()
{
  ...
  GetOutputList()->Add(hist_deltaphi);
  GetOutputList()->Add(hist_ptleptons);
  GetOutputList()->Add(hist_Hmass);
  ...
  // Add trailing Lepton histograms
  GetOutputList()->Add(hist_trailleptpt);
  GetOutputList()->Add(hist_traillepteta);
  GetOutputList()->Add(hist_trailleptE);
  GetOutputList()->Add(hist_trailleptphi);
  GetOutputList()->Add(hist_trailleptch);
  GetOutputList()->Add(hist_trailleptID);
  GetOutputList()->Add(hist_traillept_ptc);
  GetOutputList()->Add(hist_trailleptetc);
  GetOutputList()->Add(hist_traillepz0);
  GetOutputList()->Add(hist_traillepd0);
...
void TopAnalysis::WriteHistograms()
{
  ...
  // Write trailing Lepton histograms
  hist_trailleptpt->Write();
  hist_traillepteta->Write();
  hist_trailleptE->Write();
  hist_trailleptphi->Write();
  hist_trailleptch->Write();
  hist_trailleptID->Write();
  hist_traillept_ptc->Write();
  hist_trailleptetc->Write();
  hist_traillepz0->Write();
  hist_traillepd0->Write();
...
void TopAnalysis::FillHistogramsTraillept( double m, float w , TString s)
{
  //Trailing lepton histograms
  if (s.Contains("hist_trailleptpt")) hist_trailleptpt->Fill(m,w);

  if (s.Contains("hist_traillepteta")) hist_traillepteta->Fill(m,w);

  if (s.Contains("hist_trailleptE")) hist_trailleptE->Fill(m,w);

  if (s.Contains("hist_trailleptphi")) hist_trailleptphi->Fill(m,w);

  if (s.Contains("hist_trailleptch")) hist_trailleptch->Fill(m,w);

  if (s.Contains("hist_trailleptID")) hist_trailleptID->Fill(m,w);

  if (s.Contains("hist_traillept_ptc")) hist_traillept_ptc->Fill(m,w);

  if (s.Contains("hist_trailleptetc")) hist_trailleptetc->Fill(m,w);

  if (s.Contains("hist_traillepz0")) hist_traillepz0->Fill(m,w);

  if (s.Contains("hist_traillepd0")) hist_traillepd0->Fill(m,w);
}
